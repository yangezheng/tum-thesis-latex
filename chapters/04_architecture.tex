...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    \node[io] (qaf) {QAF entry \\ (raw text)};
    \node[agent, right=of qaf] (mpn) {MPN\\Extraction\\Agent};
    \node[agent, right=of mpn] (api) {Retrieval\\Agent\\(Octopart API)};
    \node[agent, below=of api] (web) {Retrieval\\Agent\\(Web search)};
    \node[agent, right=of api] (parser) {Parsing\\Agent\\(VLM)};
    \node[agent, right=of parser] (validator) {Validation\\Module};
    \node[datastore, right=of validator] (db) {Spec DB};
    \node[agent, below=of validator] (cost) {Cost\\Model};
    ...
    \draw[->] (validator) -- (db);
    \draw[->, dashed] (validator.west) -- ++(-0.8,0) |- node[left]{retry / fallback}(web.south);
    \draw[->] (validator) -- (cost);
  \end{tikzpicture}
...
\subsection{Agent Coordination and Memory}
